...
export function LoginPage() {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center p-6">
      <div className="w-full max-w-md rounded-xl border bg-[--color-bg] p-8 shadow-md">
        ...
        {showDev && (
          <form onSubmit={onDevSubmit} className="mt-6 space-y-3">
            <label className="block">
              <span className="text-xs font-medium uppercase text-[--color-muted]">
                Логин
              </span>
              <Input
                {...form.register("login")}
        ...
        {!showDev && !showTelegram && (
          <div className="mt-6 rounded-md bg-[--color-subtle] p-4 text-sm">
        ...
        {error && (
          <p className="mt-4 rounded-md bg-[--color-danger]/10 p-3 text-sm text-[--color-danger]">
            {error}
          </p>
        )}
      </div>
