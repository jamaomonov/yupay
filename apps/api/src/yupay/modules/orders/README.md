...
| Table          | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `orders`       | Either `user_id` or `guest_email` is set (CHECK enforced). `purpose` is `catalog` (storefront sale) or `wallet_topup` (1:1 deposit, no line items, ADR-0058). Customer `GET /orders` hides funding rows. Per-actor partial UNIQUE on `idempotency_key`. `total_charged` is the sum of the per-line charges, rounded to the currency's smallest chargeable unit (whole so'm for UZS, kopecks for RUB) so it is always an exact tiyin amount acquirers accept. It is **not** `total_usd × rate`: a variable-amount line is charged at `market_rate × sku.rate_multiplier`, and an override-priced line ignores the rate entirely. |
