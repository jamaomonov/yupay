...
- `"history"` — есть строка `supplier_price_history`; `latest_cost_usdt` /
  `captured_at` — из неё.
- `"current"` — строки истории нет, но именно на этого поставщика
  маршрутизирован SKU (`integrations.cost_refresh._is_routed_supplier` — та
  же проверка, которой сам `cost_refresh` решает, кому можно писать
  `Sku.cost_usdt`, ADR-0083 Decision 1); `latest_cost_usdt` — это
  `Sku.cost_usdt`, `captured_at` — `None` (это не точечный снимок во
  времени).
- `None` — ни истории, ни маршрутизации: цена этого поставщика нам
