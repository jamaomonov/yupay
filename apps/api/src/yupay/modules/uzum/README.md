...
- Amounts are **tiyin** (integer minor units). Our `order.total_charged` is
  a `Decimal` in major UZS units; the expected amount is
  `int(total_charged * 100)`, required to be an **exact** integer — any
  fractional-tiyin mismatch is `10011`, never silently rounded.
