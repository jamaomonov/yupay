...
| `gengine` | `GET /users/balance`, сумма из `order.price` самого заказа | да, но формулировка — **догадка**                                                      |
...
**Оставшийся пробел: подарки Steam** (`gengine_gifts.py`). Там один вызов
`create_gift_order` и создаёт, и платит, а цену пакета до создания мы не
знаем — предпроверить нечего. Нехватка баланса там всё ещё приходит обычным
провалом. Закрывать это — значит тянуть цену пакета из каталога в момент
покупки; не сделано осознанно.
