...
> не возвращает деньги сам (решение владельца, 2026-09-07), какое из
> сопоставлений опирается на **поле поставщика**, а какое — на **фразу в его
> документации**, и почему операторского re-grade нет ни в одну сторону.
...
- Уверенность у адаптеров **разная**, и её не сглаживают: `gengine` читает
  ...
  документации, поэтому все терминальные провалы `g2b` = `UNKNOWN`
  (см. `_FAILURE_MONEY_OUTCOME` в `g2b.py`).
- **Кто на это действует (M3b Task 3).** `_settle_merchant_deposit` —
  ...
  **до** импорта `merchants`, потому что `INVENTORY_FAILURE_MONEY_OUTCOME`
  = `RETURNED` срабатывает на каждом розничном заказе с пустым складом.
