...
- Уверенность у адаптеров **разная**, и её не сглаживают: `gengine` читает
  ...
  - `_REJECTED_UNBILLED` = `RETURNED` (M3c) — **один** отказ уже ушедшего
    ...
    **четвёртый род доказательства**: не поле, не наблюдённое поведение и не
    ...
    узко — статус, их конверт и форма сообщения
    (`_is_an_unbilled_player_rejection`), потому что ложное срабатывание
    **здесь** возвращает реселлеру деньги, которые мы, возможно, потратили.
    Отказ, который предикат не узнал, остаётся `UNKNOWN`: это и есть
