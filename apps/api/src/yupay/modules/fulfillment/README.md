...
- **Кто на это действует (M3b Task 3).** `_settle_merchant_deposit` —
  ...
  **до** импорта `merchants`, потому что `INVENTORY_FAILURE_MONEY_OUTCOME`
  ...
  Сид вызывается **вне** SAVEPOINT'а `drain_pending_tasks`: его аварийная
  ветка пишет `UNKNOWN`, и возврат, упавший внутри, откатил бы тот самый
  `RETURNED` и сделал бы возвратный провал невозвратным навсегда.
