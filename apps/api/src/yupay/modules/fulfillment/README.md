...
- **И заказ закрывается (M3c Task 6).** Если после проводки списание вернулось
  **целиком** (`merchants.refund.settled_in_full`, тот же предикат, что у
  `failure_reason` и у алерта отмены), `end_a_refunded_merchant_order` в том
...
deposit_already_returned`), так что «в работе» было ложью. Частичное
  возмещение заказ не закрывает: там человек посреди решения. **С M3c Task 4 у
  этой функции второй вызывающий** — `merchants.deposit.credit_deposit`: расчёт,
  который проводит человек, закрывает заказ ровно так же. Закрытие живёт у
  проводки, а не у кнопки в админке, иначе `curl` из ранбука и кабинет мерчанта
  (M4) продолжали бы оставлять заказ открытым. Розницы это не
  касается дважды — гейт `merchant_id` и отсутствие депозитного списания,
  против которого возмещение могло бы быть полным.
- **И задача уходит из очереди оператора (2026-09-20).** Возвращённая задача
  остаётся `failed` намеренно: `_settle_merchant_deposit_inner` гейтится ровно
  на этом статусе, то есть возврат происходит _потому что_ задача упала, и
  менять статус нельзя. Но Fulfilment Inbox — это «задачи, требующие внимания
  оператора», а на такой строке любая кнопка отвечает `409
deposit_already_returned`. Поэтому листинг задач отдаёт `deposit_settled`
  (роут считает его батчем на страницу через `deposit.refunded_for_orders` +
  `charged_for_orders`, по тому же предикату `refund.settled_in_full`, что
  закрывает заказ), а вкладка Failed такие строки прячет. Под «Все» строка
  остаётся: это сокрытие сделанной работы, а не истории. Частичный расчёт не
  прячется — там человек посреди решения. **До этого на проде висели две
  строки Dodsonshop, которые нельзя было ни выполнить, ни убрать.**
...
| `nova`              | функционален, резерв (ADR-0081) | игровые топ-апы (`kind='game'`), маршрут только через `force_supplier` — sourcing сам туда не переключает. **Деньги списываются на `create`**, поэтому провал на вызове создания или после него — `UNKNOWN`, а не `RETURNED` (как у Waxpeer, не как у G-Engine); отказ, похожий на нехватку баланса, — не провал вовсе, а `supplier_low_balance` стойло. |

Эта таблица — набор слотов из скелета ADR-0013; `g2b`, `gengine` и `waxpeer`
такие же реальные поставщики, зарегистрированные позже, и разобраны отдельно
в разделе про `MoneyOutcome` выше, а не здесь.
...
## Кто умеет распознать нехватку баланса

Три адаптера, и не одинаково:

| Поставщик | Предпроверка                                                                                               | По отказу                                                                              |
| --------- | ---------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `g2b`     | `GET /v1/getMe`, сумма из `Sku.cost_usdt`                                                                  | да, по телу ошибки                                                                     |
| `nova`    | нет, и не нужна — см. ниже                                                                                 | да, по телу ошибки, **проверено на живом отказе** (парное правило, см. `nova_grading`) |
| `gengine` | да, на всех четырёх путях: из `order.price` самого заказа, а для подарков — из `supplier_price_usd` строки | да, но формулировка — **догадка**                                                      |

У G-Engine предпроверка точнее, чем у G2B, потому что их заказ создаётся
неоплаченным и несёт свою цену: сравнивать есть с чем, гадать по себестоимости
SKU не нужно. Зато распознавание **по тексту отказа** у него самое слабое — их
собственной фразы мы ни разу не видели. Боевой заказ `01a0c95b` 2026-09-22
отказал на оплате, и в задаче осталось только `g-engine HTTP 400`: тело ответа
с их `message` никуда не писалось. Оператор пополнил кошелёк и нажал Retry
руками, потому что алерт не назвал ни причину, ни сумму.

Поэтому у `gengine` предпроверка — это механизм, а разбор текста — страховка,
а не наоборот. Проба делается best-effort: **любой** её сбой проваливается
дальше к настоящему вызову, потому что упавшая проба не должна отменять
продажу, которую мы могли совершить.

**Почему у NOVA предпроверки нет и это не тот же пробел.** Их заказ
создаётся и оплачивается одним вызовом, поэтому отказ по деньгам ничего не
оставляет за собой — ни резерва, ни лишнего раунда к дорогому эндпоинту. У
G-Engine это две фазы, и отказ на второй оставляет зарезервированный заказ.
Плюс их формулировку мы видели вживую (2026-09-18, запрос на $39.78 при
кошельке $9.10), так что разбор текста у NOVA стоит на доказательстве, а не на
догадке. Баланс для алерта NOVA читает **после** отказа (`_balance_or_none`) —
одно лишнее чтение на уже провалившемся пути.

С 2026-09-22 NOVA заполняет и вторую половину строки «Баланс: $X · Нужно: $Y»:
`_required_or_none` берёт `Sku.cost_usdt` и умножает на количество позиции.
Умножение имеет смысл только для Fragment Stars — на остальных путях NOVA
отказывает при `qty > 1`, потому что один вызов покупает одно предложение.

**Подарки Steam тоже предпроверяются** (`gengine_gifts.py`). Там
`create_gift_order` покупает и платит одним вызовом, то есть второго шага, на
котором можно отказать, не существует — предпроверка там единственный шанс
превратить пустой кошелёк в «пополни и повтори» вместо провала. Сумма берётся
из `fulfillment_data["supplier_price_usd"]`: `gifts.checkout` разрешает
позонную цену пакета в момент оформления и кладёт её в строку (она скрыта от
клиентских DTO — по ней читается наша маржа). Цена может устареть, и это
нормально: предпроверке достаточно быть правой в утверждении «столько у нас
точно нет», а дрейф checkout и так ограничивает 2%.
