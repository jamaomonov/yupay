...
class GEngineFulfiller(Fulfiller):
    ...
    async def fulfill(
        ...
        *,
        ...
        if denomination_id is None:
            ...
            params["Quantity"] = str(await quantity_for(db, item=item, mapping=mapping))
    ...
    async def _fulfill_shop(self, *, mapping: Any, item: OrderItem) -> FulfillResult:
        ...
        short = await _afford(client, reserved.price)
    ...
    async def _shop_status(self, task: FulfillmentTask) -> FulfillStatus:
        ...
        try:
            ...
            if order.status == "pending":
                ...
                # make cheap. Settle it now, if we can afford to: this is the
                # tick a wallet that emptied since the reservation is caught
                ...
                if short is not None:
                    ...
                    return _as_status(
                        _low_balance_result(
                            ...
                            mapping=None,
                            source="gengine.balance/shop_status",
                        )
                    )
            ...
            if _looks_like_low_balance(exc):
                return _as_status(
                    _low_balance_result(
                        ...
                        required=None,
                        mapping=None,
                        source="gengine.refusal/shop_status",
                        supplier_message=exc.body,
                    )
                )
        ...
        if order.status == "verified":
            ...
            if short is not None:
                ...
                return _low_balance_result(
                    ...
                    required=order.price,
                    mapping=None,
                    source="gengine.balance/recharge",
                )
            try:
                order = await client.pay_recharge_order(order.id)
            except GEngineError as exc:
                # Paying is where our money leaves. A refusal here is worth
                # surfacing verbatim rather than as a generic failure.
                if _looks_like_low_balance(exc):
                    # The balance probe above said we could afford it, so either
                    # it could not answer or the wallet moved between the two
                    # calls. Same soft landing either way.
                    return _low_balance_result(
                        current_balance=None,
                        required=order.price,
                        mapping=None,
                        source="gengine.refusal/recharge",
                        supplier_message=exc.body,
                    )
