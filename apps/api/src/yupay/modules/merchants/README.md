...
| The cabinet's Orders list (no machine-API twin)             | `cabinet_orders.py` — keyset paging, a closed filter set, money from the ledger, and `_sku_labels` for the product's name (`sku_code`/`sku_name`/`brand_name`, one query, no per-row lookup)                                                                                                                                                                                                                                            |
| The cabinet's order detail (`GET /orders/{id}`)             | `cabinet_orders.read_detail` — wraps `order_status.read` (the same reader the machine API polls) and adds the product's name on top, into `cabinet_schemas.CabinetOrderDetailOut` rather than growing the machine contract's `MerchantOrderStatusOut`                                                                                                                                                                                   |
