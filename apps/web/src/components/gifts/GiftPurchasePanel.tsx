...
import { GiftRecipientField } from "./GiftRecipientField";
...
export function GiftPurchasePanel({
  ...
  // The invite link itself is owned here, not in `GiftRecipientField`: it is
  // what `handleBuy` sends as `invite_url` and what the confirm dialog names.
  // Everything about *asking* for it — the debounced link error, the
  // «Проверить» button, the confirmed-recipient card — lives in that field.
  const [inviteUrl, setInviteUrl] = useState("");
  // Whether the field points at an accepted Steam link at all. The gate the
  // buy button and `payHint` reason off, unconditionally — the same
  // accept/reject test the field's own error paragraph is timed against.
  const inviteValid = isValidInviteUrl(inviteUrl);
  const inviteHasValue = inviteUrl.trim() !== "";

  // The pre-purchase recipient check («Проверить»), as reported by the field:
  // one verdict, filed under the profile it was asked about, and read back
  // only while the link still points at that same profile (`currentVerdict`).
  //
  // Kept up here rather than inside the field with the rest of its state,
  // and reported synchronously rather than mirrored up from an effect the way
  // `CheckablePlayerField` does it: this verdict gates Buy, and an effect
  // lands a commit later — long enough for the field to paint a `not_found`
    ...
    <>
      <div ref={panelRef} className="border-border bg-card space-y-5 rounded-2xl border p-5 sm:p-6">
        ...
        <GiftRecipientField
          ...
          check={profileCheck}
          onCheckResult={setRecipientVerdict}
        />
