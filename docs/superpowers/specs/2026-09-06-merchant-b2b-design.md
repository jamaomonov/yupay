...
| Товары       | tabs Топапы · Ваучеры; per SKU: this-merchant price, availability, API id, copy-paste request example; CSV/JSON export                                                                                                                                                                                                                                                                                                                                                    |
| Заказы       | filters, search by `merchant_order_id` / our id; detail: timeline, code reveal, failure reason, refund mark                                                                                                                                                                                                                                                                                                                                                               |
| Ручной заказ | place an order from the cabinet without code _(owner)_: pick a SKU (search across the B2B catalog), fill the same `fulfillment_data` fields the API takes, see the exact price, confirm — debited from the deposit like any API order. Goes through the same pricing/validation path; the BFF mints `manual-<uuid>` as the `merchant_order_id`, so idempotency and history stay uniform. This is also how a merchant makes their first test purchase without writing code |
