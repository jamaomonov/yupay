...
| `-31008` | Operation not valid for the transaction's current state (e.g. two Creates racing, or Perform on a cancelled tx) | Usually a client-side retry race; check `CheckTransaction` for the real state             |
