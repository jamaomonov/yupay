...
| Game task `failed`, `last_error: g2b game order failed: HTTP 400: {"message":"Invalid player ID…`                  | The player id does not exist for that game — G2B refused before charging us                                               | Nothing to chase at G2B. **Verify, do not assume:** `last_error` is a 200-char slice while the grader reads up to 500 and also requires `"success": false`, so read the task's `money_outcome`. `returned` on a **merchant** order means the deposit came back by itself (check non-zero `refunded_usd`; the reseller must then place a **new** order — Retry is refused). `unknown` means it parked: settle it by hand per `merchant-b2b.md`. A retail order is refunded by hand either way |
