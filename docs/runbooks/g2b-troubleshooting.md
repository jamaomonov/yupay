...
| Game task `failed`, `last_error: g2b game order failed: HTTP 400: {"message":"Invalid player ID…`                  | The player id does not exist for that game — G2B refused before charging us                                               | Nothing to chase at G2B. A **merchant** order refunds its deposit by itself (`money_outcome=returned`, M3c); a retail order is refunded by hand as usual |
