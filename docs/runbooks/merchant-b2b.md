...
## SEO/AEO: что смотреть

**Чек-лист перед деплоем:** в Cloudflare «Block AI Scrapers and Crawlers»
выключен на зонах `yupay.uz` и `reseller.yupay.uz`, `reseller.yupay.uz` не
под Bot Fight Mode; пересобрать образы **обоих** приложений — `merchant` и
`web` (ссылка в подвале сторфронта на программу и раздел про неё в
`llms.txt` уходят в одном и том же изменении); прогнать 12-промптный замер
ниже **до** любого анонса.

Реселлерский сайт (`reseller.yupay.uz`, `apps/merchant`) с 2026-09-17 имеет
SEO-слой, перенесённый с розницы (`sitemap.xml`, `robots.txt` с AI-ботами,
JSON-LD, `llms.txt`) — см. [ADR-0080](../decisions/0080-reseller-site-for-non-technical-resellers.md)
и `docs/superpowers/specs/2026-09-17-reseller-seo-design.md` §8. Ниже —
панель измерения оттуда, скопированная сюда как единственный источник
правды по вопросу «работает ли это».

**Первый замер — до того, как страницы анонсируются; результаты — в этот
раздел датой.**

### Панель промптов — раз в месяц, вручную, 20 минут
...
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| **Google Search Console** | Добавить `reseller.yupay.uz`. Если у `yupay.uz` домен-property (верификация по DNS), поддомен уже покрыт, но отдельная URL-prefix property даёт свой отчёт | Запросы §1.1/§1.3, средняя позиция по «оптом» и «api пополнения», покрытие `/telegram`, `/api`, `/faq` и `/docs/*` |
| **Bing Webmaster Tools**  | **Обязательно.** Добавить хост, загрузить sitemap                                                                                                          | Единственный рычаг к ChatGPT. Импорт из GSC — в один клик                                                          |
