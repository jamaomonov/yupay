...
| A new supplier integration                 | `apps/api/src/yupay/modules/fulfillment/suppliers/<vendor>.py` implementing the `Fulfiller` protocol (`fulfillment/suppliers/base.py`). Register in `REGISTRY`, `fulfillment/suppliers/__init__.py` — g2b, gengine, nova and waxpeer are the existing examples. (`integrations/adapters/` and `integrations/registry.py`, named here in an earlier revision, do not hold supplier code — `integrations/adapters/` is an empty, unused directory and `integrations/registry.py` does not exist; `integrations` owns the SKU↔supplier _mapping_ and cost sync, not the adapters themselves.) |
