...
#
# Production image for the partner site (partners.yupay.uz).
#
...
ARG NEXT_PUBLIC_STOREFRONT_URL=https://yupay.uz
ENV NEXT_PUBLIC_STOREFRONT_URL=${NEXT_PUBLIC_STOREFRONT_URL}
RUN pnpm --filter @yupay/partners build
...
EXPOSE 3003
HEALTHCHECK --interval=30s --timeout=3s CMD wget -qO- http://localhost:3003/ || exit 1
CMD ["pnpm", "--filter", "@yupay/partners", "start"]
